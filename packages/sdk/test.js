...
const paymentInput = sdk.readJson(
  path.join(root, "testdata/eligibility/input.valid.json"),
);
const paymentPublic = sdk.readJson(
  path.join(root, "testdata/eligibility/public.json"),
);
const paymentCliArgs = sdk.readJson(
  path.join(root, "testdata/eligibility/cli-args.json"),
);
...
const gatePaymentSpec = sdk.readJson(
  path.join(root, "apps/web/data/gate-payment-spec.json"),
);

assert.deepStrictEqual(manifest.exports["./react"], {
...
assert.throws(
  () => sdk.assertPaymentAction(rwaInput, rwaPublic),
  /payment proof/,
);

assert.deepStrictEqual(
  sdk.formatSorobanPubSignals(paymentPublic),
  paymentCliArgs.pub_signals,
);
assert.deepStrictEqual(
  sdk.formatImplicitCliPubSignals(paymentCliArgs.pub_signals),
  paymentPublic,
);
assert.strictEqual(
  typeof sdk.formatBindingPubSignals(paymentPublic)[0],
  "bigint",
);
...
assert.strictEqual(
  paymentInvoke.packet_hash,
  BigInt(sdk.parsePublicSignals(paymentPublic).packet_hash),
);
...
assert.strictEqual(
  rwaInvoke.terms_hash,
  BigInt(sdk.parsePublicSignals(rwaPublic).packet_hash),
);
