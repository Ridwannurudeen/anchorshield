...
**P3.3 Repeatable-demo nullifier.** The browser no longer serves a fixed witness from `apps/web/data`; operators load a local witness JSON before proving. To make the live submit repeatable, **vary a nullifier-affecting private input per session** (e.g. randomize `epoch` or `user_secret`/an entropy field in the witness before proving), provided the gate/policy does not pin that field.
