...
- **#5 real on-chain deploy — IN PROGRESS.** Deploy the hardened contract set + OZ SEP-57 stack to testnet with the ceremony VK and capture real `attest`/`mint`/`verify_and_pay` transactions; update `deployments/testnet.json` and remove the historical-fixture caveat below.
